...
def final_report(data,avg_price,avg_dis,na_type,dup_type,filename):
    ...
    try:
        ...
        data[['Actual Price(EUR)','Discounted Price(EUR)']] = data[['Actual Price(EUR)','Discounted Price(EUR)']].astype(float)
        # data['Savings(%)'] = data['discount_percentage']*100
        # data['Savings(%)2'] = np.where((data['Actual Price(EUR)'] != 0),data['discount_percentage']*100,data['Actual Price(EUR)'])

        data['Savings(%)'] = np.where((data['Actual Price(EUR)'] != 0) & (data['Discounted Price(EUR)'] !=0),((data['Actual Price(EUR)'] - data['Discounted Price(EUR)'])/(data['Actual Price(EUR)'])) * 100,data['Actual Price(EUR)'])
...
if __name__== "__main__" :
    ...
    folder_path = r'C:\\Users\\LENOVO\\..\\data'
    ...
    df_cd,cd_check = create_dataframe('carier_details',carier_details)
    if cd_check == 1:
        print('Invalid details in carier_details excel file')
        sys.exit(1)

    df_cpd,cpd_check = create_dataframe('carrier_price_details',carrier_price_details)
    if cpd_check == 1:
        print('Invalid details in carrier_price_details excel file')
        sys.exit(1)

    df_crcy,crcy_check = create_dataframe('currency',currency)
    if crcy_check == 1:
        print('Invalid details in currency excel file')
        sys.exit(1)

    df_cust_d,cust_d_check = create_dataframe('customer_details',customer_details)
    if cust_d_check == 1:
        print('Invalid details in customer_details excel file')
        sys.exit(1)

    df_od,od_check = create_dataframe('order_details',order_details)
    if od_check == 1:
        print('Invalid details in order_details excel file')
        sys.exit(1)

    df_para,para_check = create_dataframe('parameter',parameter)
    if para_check == 1:
    ...
    df_pd,pd_check = create_dataframe('product_details',product_details)
    if pd_check == 1:
